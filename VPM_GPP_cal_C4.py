...
#-------------------------------------------------------------------------------
# Name:        NorthAmerica GPP simulation using the VPM model, MODIS collection6 data and cdl layer
...
def buildVrtFile (root,year,tile,product):
    ...
    filename=[]
    filename.append(os.path.join('/data/ifs/users/xcwu/VPM_GPP/TEMP/VRT',year+tile+product+'_list.txt'))  ######
...
def GPPprocess(tile):
    ...
    dirEVI=root+'/VPM/driving_data/MOD09A1_006_BISE_SG/'+year+'/'+tile   ####EVI data, gap filled using Yao's BISE method
    #dirEVI=root+'/modis/products/mod09a1/geotiff/evi/'+year+'/'+tile
	#########this is for C6 version but using C5 LSWI for 2000.
    dirLSWI=root+'/modis/products_006/mod09a1/geotiff/lswi/'+year+'/'+tile  ###MODIS C6 
    dirC4 = root+'/users/xcwu/CDL/process/NorthAmerica/cdldata/'+year+'_30m/C4/'+tile+'_cdl_C4_Perc.img'  #############aggregated C4 layer
    #climate data R and T
    fileRAD = root+'/users/xcwu/CDL/process/NorthAmerica/climate/radiation/'+year+'/rad.'+tile+'.'+year+'.nc'
    fileTEMP = root+'/users/xcwu/CDL/process/NorthAmerica/climate/temperature/'+year+'/temp.'+tile+'.'+year+'.nc'
    ...
    if (LC!=None and LSWImax!=None):
        ...
        dirGPPTILE = root+'/users/xcwu/CDL/process/NorthAmerica/VPM_output_June/GPP/'+year+'_30m/'+tile
        dirC4GPPTILE = root+'/users/xcwu/CDL/process/NorthAmerica/VPM_output_June/C4_GPP/'+year+'_30m/'+tile
        #dirEGTILE = root+'/users/xcwu/CDL/process/NorthAmerica/VPM_output/'+year+'/'+tile
        
        if not os.path.exists(dirGPPTILE):
            os.makedirs(dirGPPTILE)
        if not os.path.exists(dirC4GPPTILE):
            os.makedirs(dirC4GPPTILE)
        #if not os.path.exists(dirEGTILE):
        #    os.makedirs(dirEGTILE)
        ...
        LUTepsilon=numpy.array([float('nan'),0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078,0.078])####all using 0.078 for C3 plants, only 1.5 times for C4 plants
        ...
        EVI=1.25*(EVI*0.0001-0.1)  ####VPM_output
        #EVI = EVI * 0.0001; ####VPM_output2
        EVI[EVI<0]=0
        EVI[EVI>1]=1
        ...
        TEMP=netCDFTEMP.variables['air'][:]*100
        TEMP=TEMP-27315 # from Kevin  to Celsius
    
        print "reading the multi-NARR radiation file..."
        netCDFRAD= Dataset(fileRAD, 'r')
        RAD=netCDFRAD.variables['dswrf'][:]
        #RAD=RAD/100 #convert the Radiation to w/m-2
        
        print "reading the aggregated C4 layer"
        inC4_Perc=gdal.Open(dirC4)
        C4_Perc=inC4_Perc.ReadAsArray()
        ...
        Wscalar=(10000+LSWI)*1.000/(10000+LSWImax)
        ...
        C4_Perc=numpy.where(C4_Perc>32700,0,C4_Perc)
    
        #scalar for APAR is 0.01 w/m2
        #APAR=EVI*RAD*100                                               #######################################################################################
        #scalar for EG is 0.0001 
        #EG=LUE*Tscalar*Wscalar*10000                                   #######################################################################################
        print "calculating GPP..."
        #scalar for GPP is 0.001 g C m-2 day-1
        #GPP=APAR*EG/1000
        GPP=RAD*0.8*EVI*Tscalar*Wscalar*LUE*(C4_Perc*1.5+(1000-C4_Perc)*1.0) ########################Scalar for GPP is 0.001 g C m-2 day-1
        C4_GPP=numpy.where(C4_Perc>0,RAD*0.8*EVI*Tscalar*Wscalar*LUE*1.5*C4_Perc,0)               ########################Scalar for C4_GPP is 0.001 g C m-2 day-1
        export_array (Rasters=numpy.uint(GPP),directory=dirGPPTILE,type='GPP',year=year,tile=tile,index=allexist)
        GPP=None
        export_array (Rasters=numpy.uint(C4_GPP),directory=dirC4GPPTILE,type='C4_GPP',year=year,tile=tile,index=allexist)
        C4_GPP=None
...
#    'h13v08','h16v08de','h17v08','h18v08','h19v08','h20v08','h21v08','h22v08','h23v08','h25v08','h26v08',\
...
tile=['h08v04','h08v05','h08v06','h09v04','h09v05','h09v06','h10v04','h10v05','h10v06','h11v04','h11v05','h12v04','h12v05','h13v04']
#tile=['h09v05']
